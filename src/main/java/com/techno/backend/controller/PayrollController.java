...
public class PayrollController {
    ...
    public ResponseEntity<SalaryDetailsResponse> getSalaryDetails(
        ...
        SalaryHeader salary = salaryHeaderRepository
                ...
                .orElseThrow(() -> new ResourceNotFoundException(
                        "لم يتم العثور على راتب للموظف " + employeeNo + " في الشهر " + salaryMonth));
    ...
    public ResponseEntity<ApiResponse<List<SalaryHeader>>> getSalariesForMonth(@PathVariable String salaryMonth) {
        ...
        if (auth != null && auth.getAuthorities().stream()
            ...
            log.info("Employee role detected, filtering to employee number: {} for month: {}", currentEmployeeNo, salaryMonth);
            
            Optional<SalaryHeader> salaryOpt = salaryHeaderRepository.findLatestByEmployeeAndMonth(currentEmployeeNo, salaryMonth);
        ...
        if (auth != null && auth.isAuthenticated()) {
            ...
            if (principal instanceof String) {
                try {
                    ...
                    log.error("Invalid employee number in principal: {}", principal);
                    throw new IllegalStateException("لم يتم العثور على رقم الموظف في سياق الأمان. الرئيسي: " + principal);
                }
            }
        }
