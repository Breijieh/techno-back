...
public interface SalaryBreakdownPercentageRepository extends JpaRepository<SalaryBreakdownPercentage, Long> {
     ...
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.employeeCategory = :category AND s.isDeleted = 'N' ORDER BY s.transTypeCode ASC")
     ...
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.employeeCategory = 'S' AND s.isDeleted = 'N' ORDER BY s.transTypeCode ASC")
    List<SalaryBreakdownPercentage> findSaudiBreakdown();

    /**
     * Find all salary breakdown percentages for Foreign employees (F)
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.employeeCategory = 'F' AND s.isDeleted = 'N' ORDER BY s.transTypeCode ASC")
    List<SalaryBreakdownPercentage> findForeignBreakdown();

    /**
     * Find specific breakdown by category and transaction type (active only)
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.employeeCategory = :category AND s.transTypeCode = :transTypeCode AND s.isDeleted = 'N'")
    ...
    /**
     * Find specific breakdown by category and transaction type (including soft-deleted)
     * Used when creating/updating to check for existing records regardless of deletion status
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.employeeCategory = :category AND s.transTypeCode = :transTypeCode")
     ...
     */
    @Query("SELECT s FROM SalaryBreakdownPercentage s WHERE s.isDeleted = 'N' ORDER BY s.employeeCategory ASC, s.transTypeCode ASC")
