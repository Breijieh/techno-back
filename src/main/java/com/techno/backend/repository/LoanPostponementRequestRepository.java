...
public interface LoanPostponementRequestRepository extends JpaRepository<LoanPostponementRequest, Long> {
       ...
       /**
        * Find all postponement requests with optional filters for status, employee,
        * and date range.
        ...
        */
       /**
        * Find all postponement requests filtered to TECHNO contract employees only.
        */
       @Query("SELECT p FROM LoanPostponementRequest p " +
                     ...
                     "(:transStatus IS NULL OR p.transStatus = :transStatus) AND " +
